...
public class Recommendation {
    ...
    public void printRecommendedMovies() {
        // Create a VBox to hold the recommended movies
        VBox vBox = new VBox();
        vBox.setPadding(new Insets(10, 10, 10, 10));
        vBox.setSpacing(10);
        
        // Create a Label for the title
        Label title = new Label("Recommended Movies:");
        title.setStyle("-fx-font-weight: bold; -fx-font-size: 16px;");
        vBox.getChildren().add(title);
        
        // Loop through the recommended movies list and add each movie to the VBox
        for (Movie movie : this.recommended_movies) {
            // Create a Label for each movie and add it to the VBox
        ...
        // Create an Alert to display the recommended movies
        Alert alert = new Alert(AlertType.INFORMATION);
        alert.setTitle(String.format("Your Recommended Movie List (%d)", this.recommended_movies.size()));
        ...
        alert.getDialogPane().setStyle("-fx-background-color: #ADD8E6;");


        // Apply CSS styling to the title
        title.getStyleClass().add("title-style");

        // Apply CSS styling to the movie labels
        vBox.getChildren().forEach(node -> {
            if (node instanceof Label) {
                Label label = (Label) node;
                label.getStyleClass().add("movie-label-style");
            }
        });
